...
describe('WhatsApp vai para o cliente do orçamento', () => {
  it('com telefone cadastrado, abre a conversa com o cliente', async () => {
    const o = novo();
    ...
    const link = await screen.findByRole('link', { name: 'Enviar no WhatsApp' });
    await waitFor(() =>
      expect(link.getAttribute('href')).toMatch(/^https:\/\/wa\.me\/5518997124455\?text=/),
    );
  });

  it('sem telefone, deixa o WhatsApp pedir o contato — e avisa no botão', async () => {
    const o = novo();
    await db.clientes.put({
      id: o.clienteId,
      nome: o.clienteNome,
    ...
    const link = await screen.findByRole('link', { name: 'WhatsApp (escolher contato)' });
  ...
  it('nunca manda para o número da própria AA Montagens', async () => {
    ...
    expect(link.getAttribute('href')).not.toContain('5518998230660');
    expect(link.getAttribute('href')).not.toContain('5518997882819');
  });
});
